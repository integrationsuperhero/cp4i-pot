...
3\. When prompted, use the username and password provided to you for this lab. The username in the screenshots of this lab is chopper9.

![alt text][pic5]

4\. Click on the **Integration runtimes** tab.

![alt text][pic6a]

If you do not see **Integration runtimes**, click on the caret to expand the section.

![alt text][pic6b]

5\. Click on the qmgr link to take you to the MQ Console.  This should be suffixed with your username that you used to sign in with. In this example, it is showing sign in as chopper9.
...
3\. In the **Workspace Launcher** window, enter **home/student/POTWorkspaceIBM**. Click **OK**.
...
5\.  If you are viewing this page from within the VDI desktop, click on the [**ace-apic-lab8.zip**](asset/ace-apic-lab8.zip) link to download the file.

6\. Click **Download** to download ace-apic-lab8.zip.
...
11\. Navigate to the directory (**student** &#8594; **Downloads**) that contains the **ace-apic-lab8.zip** file and click **OK**.

![alt text][pic18]

12\. Confirm that the **ace-apic-lab8.zip** file is listed in the **From zip file** field, confirm that the **Project location root** contains the path to your workspace (**home/student/POTWorkspace**), confirm there is a checkmark next to **orders**, and click **Finish**.
...
17\. When prompted, use the username and password provided to you for this lab. The username in the screenshots of this lab is chopper9.

![alt text][pic128]

18\. Close the **Welcome to Dev Perspective!** window.

![alt text][pic129]

19\. Click on the caret next to **Developer** and select **Administrator**.
...
21\. You will be presented with a screen that says restricted access.

![alt text][pic132]

Click on the caret next to **Projects: all projects** and select the project that has the same name as the username that you logged in with.  The project in the screenshots for this lap is chopper9.

![alt text][pic133]

22\. Screen down until you see the services prefixed with mq.  The service that you will use in the next step is **mq-<span style="color: red">username</span>-ibm-mq**.
...
24\. Select **MQ client connection properties** from the drop-down menu for **Connection**.  Enter the name of your queue manager for **Destination queue manager name**, **Queue manager host name** (<service-name>.<namespace>.svc), **Listener port number** (1414), and **Channel name** (SYSTEM.DEF.SVRCONN).
...
27\. Enter the name of BAR file: **<span style="color: red">username</span>_orders** and click **Finish**. App Connect Enterprise is creating an empty BAR file. In the example below, the username is chopper9. You will want to replace chopper9 with your username.
...
Click on the caret next to **Run** and select **Integrations**.
...
3\. In the **Create an Integration Server** page. You have two options to deploy a BAR file.  We can deploy a BAR file from App Connect Toolkit or from App Connect Designer. In this lab, we will deploy a BAR file from App Connect Toolkit. Click **Quick start toolkit integration** and click **Next**.
...
6\. The final section is the server details. We will give it a name (**<span style="color: red">username</span>-orders**), enable the toggle for **Enable Operations Dashboard tracing**, and enter **cp4i-tracing** for the **Operations Dashboard namespace**.  In the example below, the username is chopper9. You will want to replace chopper9 with your username. Click **Create**.

![alt text][pic36]

7\. This will take you back to the Servers Dashboard where you will see your new server. It will likely be showing **Pending** while it is starting up the pod.

![alt text][pic37]

8\. **Note:** It may take a several minutes to start up.  You can refresh the page. Once it is up and running it will show the following:

![alt text][pic39]

9\. We can also quickly test the API call running in the Integration server.  Click on the **<span style="color: red">username</span>-orders** Server.  In the example below, the username is chopper9.
...
13\. Let's check to see if the message is in the MQ queue.  Select your Queue Manager instance. This should be suffixed with your username that you used to sign in with. In this example, it is showing sign in as chopper9.
...
1\. In the Platform Navigator, select your App Connect Dashboard instance. This should be prefixed with your username that you used to sign in with. In this example, it is showing sign in as chopper9.
...
3\. Click on the **<span style="color: red">username</span>-orders** Server.  In the example below, the username is chopper9.
...
9\. In the Platform Navigator, select the **apic-min** instance under **API Connect**.
...
11\. When prompted, log in with the username and password provided to you for this lab. Click **Log in**. The username in the screenshots of this lab is chopper9.
...
19\. In the **API Setup** tab, we can configure the API.  Here we can explore the **Design**, **Source**, and **Assemble** tabs. To test the API, we must activate the API. Switch the toggle from **Offline** to **Online** and click **Save**. This step automatically publishes the API.
...
1\. If the Platform Navigator, expand the menu in the upper left.
...
6\. Click on one of the Trace entries. **Note:** Your trace entries may look different than what is captured in the screenshot below.
...
2\. Select **apis-minimum** under **API Lifecycle management**.

![alt text][pic138]

3\. Click on **Developer APIs and products**.

![alt text][pic96]

4\. Click on the triple-dot icon for **orders** and click **Delete**.
...
1\. In the Platform Navigator, select **Home**. 
...
2\. Under **Integrations**, select your App Connect Dashboard instance. This should be prefixed with your username that you used to sign in with. In this example, it is showing sign in as chopper9.
...
1\. In the Platform Navigator, select your Queue Manager. This should be suffixed with your username that you used to sign in with. In this example, it is showing sign in as chopper9.
