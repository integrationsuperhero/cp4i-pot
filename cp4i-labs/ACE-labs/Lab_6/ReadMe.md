...
For this Lab we will use the Open API Document (servicenow-0.0.1.yaml) we downloaded from our ace integration server earlier in order to import the API.
...
3\. On the Import API page either drag and drop the yaml file from the ServiceNow Designer lab or click to upload it.
Click Next.
...
* Make sure the **Base Path** section matches the base path of your API **( /ServiceNowRetrieve )**
...
3\. Now let us look at the SystemUserID path.  From the edit page click on the GET operation to open a new page and view its configuration.
...
* Click the Add Definition icon The Add Definition icon.
...
**Note: make sure that it has http:// at the beginning.**
...
5\. On the test page you will see all the operators that are in this API. We have the users and the SystemUserID.   We will select the users GET and click on the Send button.  
...
7\. You can also test the SystemUserID operator but using one of the ids from the previous call.

![alt text][pic29]

The Response will return just the one SystemUserID
...
3\. If you didn’t publish the Salesforce API then we will need to create a new Product. If you did already publish the Salesforce API go ahead and select Existing Product. Then click next
...
Lets first click on Products and select the product name.

![alt text][pic35]

6\. It will take you to the page where you can review varies items on the menu to the right.
...
**Note: if you already created a account use that to login.**
...
10\. Now from the Products page we see our API for ServiceNow Users and below that you see Plans.  This can be configured back in the Products section where you may add plans for Silver and Gold plans.   We have just the default for this one. Click on the API tile.
...
**If you already registered an application for the Salesforce lab that will be populated in the client id and secret for you and you can go to step 16**

**In this example we have already subscribed to the Demo Accounts application**
...
Click the Send several times to invoke the API.
